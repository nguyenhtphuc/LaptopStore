(function ($) {
  ...
  var loadMyCartEvent = function(userOptions){
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
          ...
          '<button type="button" class="btn btn-success btn-thanh-toan">Thanh toán</button>' +
          '</div>' +
          '</div>' +
          '</div>' +
          '</div>'
  ...
  $(document).on('click', ".btn-thanh-toan", function() {
    ...
    if (isLoggedIn === 'true') {
        ...
        localStorage.setItem('totalPrice', ProductManager.getTotalPrice());

        window.location.href = 'thanhtoan.html?cart=' + encodeURIComponent(JSON.stringify(products));
    } else {
    ...
    var drawTable = function(){
      ...
      $cartTable.append(products.length ?
        ...
        '<td></td>' +
        '</tr>'
        : '<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">Giỏ hàng trống</div>'
      );
      ...
      if(products.length && discountPrice !== null) {
        $cartTable.append(
          '<tr style="color: red">' +
          '<td></td>' +
          '<td><strong>Tổng tiền (sau khi đã được giảm giá)</strong></td>' +
          '<td></td>' +
          '<td></td>' +
    ...
    */
    ...
    $(document).on("input", "." + classProductQuantity, function () {
      ...
      $(this).parent("td").next("." + classProductTotal).text(price * quantity + "VND");
