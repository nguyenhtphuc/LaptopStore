...
function layThongTinSanPhamTuTable(id) {
    ...
    var color = tr[12].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var sz = tr[13].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var material = tr[14].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var style = tr[15].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var overview = tr[16].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var care = tr[17].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var size = tr[18].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    var brand = tr[19].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
...
function addKhungSuaSanPham(masp) {
    ...
    var company = ["Uniqlo", "Adidas", "Nike", "Prada", "Dior", "Supreme","Gucci", "Gap", "Zara", "CalvinKlein", "Hermes", "LouisVuitton","Levi's", "Chanel", "H&M"];
    ...
    s += `
        ...
        <tr>
            <th colspan="2">Thông số sản phẩmt</th>
        </tr>
        <tr>
            <td>Mô tả:</td>
            <td><input type="text" value="`+sp.detail.description+`"></td>
        </tr>
        <tr>
            <td>Màu sắc:</td>
            <td><input type="text" value="`+sp.detail.color+`"></td>
        </tr>
        <tr>
            <td>Kích cỡ:</td>
            <td><input type="text" value="`+sp.detail.sz+`"></td>
        </tr>
        <tr>
            <td>Chất liệu:</td>
            <td><input type="text" value="`+sp.detail.material+`"></td>
        </tr>
        <tr>
            <td>Kiểu dáng:</td>
            <td><input type="text" value="`+sp.detail.style+`"></td>
        </tr>
        <tr>
            <td>Tổng quan:</td>
            <td><input type="text" value="`+sp.detail.overview+`"></td>
        </tr>
        <tr>
            <td>Hướng dẫn giặt:</td>
            <td><input type="text" value="`+sp.detail.care+`"></td>
        </tr>
        <tr>
            <td>Size mẫu:</td>
            <td><input type="text" value="`+sp.detail.size+`"></td>
        </tr>
        <tr>
            <td>Thương hiệu:</td>
            <td><input type="text" value="`+sp.detail.brand+`"></td>
        </tr>
        <tr>
    ...
    for(var i = 0; i < u.length; i++) {
        for(var j = 0; j < u[i].donhang.length; j++) {
            ...
            for (var s of u[i].donhang[j].sp) {
                var product = timKiemTheoMa(list_products, s.ma);
                if (product && product.promo && product.promo.name === 'giareonline') {
                    tongtien += stringToNum(product.promo.value);
                } else {
                    tongtien += stringToNum(product.price);
                }
            }            
            // Hàm này trả về giá trị của thuộc tính "price" của một sản phẩm dựa trên mã sản phẩm
function getPriceByProductCode(productCode) {
    // Tìm kiếm sản phẩm trong danh sách sản phẩm
    var product = timKiemTheoMa(list_products, productCode);
    
    // Kiểm tra nếu sản phẩm tồn tại và có thuộc tính "price"
    if (product && product.hasOwnProperty('price')) {
        return product.price; // Trả về giá trị của thuộc tính "price"
    } else {
        return null; // Trả về null nếu không tìm thấy hoặc không có thuộc tính "price"
    }
}

// Sử dụng hàm để lấy giá trị của "price" của một sản phẩm
var productCode = 'ABC123'; // Mã sản phẩm cần tìm giá
var price = getPriceByProductCode(productCode);

// Kiểm tra nếu giá trị được trả về là hợp lệ
if (price !== null) {
    console.log('Giá sản phẩm', productCode, 'là:', price);
} else {
    console.log('Không thể đọc được giá của sản phẩm', productCode);
}
