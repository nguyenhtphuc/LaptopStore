...
function logIn(form) {
    ...
    for (var ad of adminInfo) {
        if (equalUser(newUser, ad)) {
            alert('Chào Admin. Bắt đầu phiên làm việc!');
...
function addHeader() {
	...
	<div class="header group">
        <div class="logo">
            <a href="index.html">
                <img src="img/logofs.jpg" alt="Trang chủ Fashion Store" title="Trang chủ Fashion Store" height=200px>
            </a>
        </div> <!-- End Logo -->
...
function addFooter() {
    ...
    <div class="copy-right">
        <p><a href="index.html">Fashion Store</a></a> - DoAnWeb © 2024- Designed by
            <span style="color: #eee; font-weight: bold">Tường Vy & Diệu Anh</span></p>
    </div>`);
}
...
function getThongTinSanPhamFrom_FashionStore() {
    javascript: (function () {
        ...
        s.innerHTML = `
			(function () {
				...
				for (var li of li_s) {
					...
					switch (loai) {
						case "Mô tả:":
							result.detail.description = giatri.replace('"', "'");
							break;
						case "Màu sắc:":
							result.detail.color = giatri;
							break;
						case "Kích cỡ:":
							result.detail.sz = giatri;
							break;
						case "Chất liệu:":
							result.detail.material = giatri;
							break;
						case "Kiểu dáng:":
							result.detail.style = giatri;
							break;
						case "Tổng quan:":
							result.detail.overview = giatri;
							break;
						case "Hướng dẫn giặt:":
							result.detail.care = giatri;
							break;
						case "Size mẫu:":
							result.detail.size = giatri;
							break;
						case "Thương hiệu:":
							result.detail.brand = giatri;
							break;
					}
				}
...
function generateProductCode() {
    ...
    var random = Math.floor(Math.random() * 1000); // Số ngẫu nhiên từ 0 đến 999
    return "PROD" + timestamp + random; // Kết hợp thời gian và số ngẫu nhiên
}

// Sử dụng hàm để tạo mã sản phẩm cho mỗi sản phẩm
